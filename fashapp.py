...
import torch
from fashion_clip.fashion_clip import FashionCLIP  # Import FashionCLIP model
import warnings
import cv2
import tempfile
import subprocess
...
def generate_fashion_labels(image):
    with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
        image.save(tmp.name)
        images = [Image.open(tmp.name)]
        image_embeddings = model.encode_images(images, batch_size=32)
        image_embeddings = image_embeddings / np.linalg.norm(image_embeddings, ord=2, axis=-1, keepdims=True)
        # You can generate text labels here or return embeddings for further processing
        # For simplicity, let's return a placeholder text for now
        return "Fashion label(s) here"
...
if uploaded_file is not None:
    ...
    for i, frame in enumerate(frames):
        ...
        with col:
            ...
            with st.spinner(f'Generating fashion labels for frame {i+1}...'):
                labels = generate_fashion_labels(frame)
                initial_captions.append(labels)
                timestamps.append(formatted_timestamp)
                st.write(f"{formatted_timestamp} - {labels}")
